...
def _random_pool_category_entry_sizes(bucket_dir: Path) -> dict[str, int]:
    ...
    return {
        "car": sum(_path_size_bytes(path) for path in car_paths if path.exists()),
        "helmet": sum(_path_size_bytes(path) for path in helmet_paths if path.exists()),
        "suit": sum(_path_size_bytes(path) for path in suit_paths if path.exists()),
    }
...
def _process_tp_showroom_accessory_targets(
    *,
    ...
    def _apply_local_for_user(user: SessionUser) -> None:
        ...
        try:
            ...
            while not stop_event.is_set() and not cancel_event.is_set():
                if reader is None:
                    ...
                    break
                poll_state, observed_session = read_session_from_sdk(reader)
...
class DownloaderUI:
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(perf, text="Safe = one online car lane at a time. Session Total = one car lane per active car group. Manual = cap the concurrent car lanes yourself. Leave the advanced box off to process cars first, then helmets, then suits.", foreground="#666666", wraplength=320, justify="left").grid(row=10, column=0, columnspan=2, sticky="w", pady=(8, 0))
        ...
        ttk.Label(online_left, textvariable=self.tp_manifest_member_id_summary_var, justify="left", foreground="#555555", wraplength=360).grid(row=6, column=0, sticky="w", pady=(6, 0))
    ...
    def _session_snapshot_signature(self, snapshot: RuntimeSnapshot) -> tuple:
        ...
        rows_sig = tuple(
            (
                ...
                row.status_code,
                row.car_status_code,
                row.suit_status_code,
                row.helmet_status_code,
            )
    ...
    def _poll_service(self) -> None:
        ...
        if not self._headless_controller_mode:
            self._maybe_autofill_ai_roster_member_id_from_session(self.service.get_runtime_snapshot())
        self._refresh_session_tree()
    ...
    def _open_quick_start_wizard(self) -> None:
        ...
        pages = []
        step_var = self.tk.StringVar(value="Step 1 of 5")
        ...
        p3 = _page_container()
        self.ttk.Label(p3, text="3 • Missing paint fallback", font=("Segoe UI", 10, "bold"), foreground="#0b63b6").pack(anchor="w")
        self.ttk.Label(
            p3,
        ...
        p4 = _page_container()
        self.ttk.Label(p4, text="4 • Trading Paints login", font=("Segoe UI", 10, "bold"), foreground="#0b63b6").pack(anchor="w")
        explain = (
        ...
        p5 = _page_container()
        self.ttk.Label(p5, text="5 • Ready", font=("Segoe UI", 10, "bold"), foreground="#0b63b6").pack(anchor="w")
        ...
        def _refresh_nav_state() -> None:
            ...
            if (needs_docs_confirmation and index >= 1) or (needs_login and index >= 3):
        ...
        def _next() -> None:
            if not bool(quick_start_docs_state.get("confirmed")) and page_index["value"] >= 1:
                try:
                    ...
                    pass
                return
            if _online_setup_required() and not _tp_login_ready() and page_index["value"] >= 3:
                try:
