...
def _session_overall_status_code(asset_statuses: dict[str, str]) -> str:
    ...
    for preferred in ("downloading", "replay", "fallback_online", "fallback_local", "fallback", "downloaded", "missing", "skipped", "queued"):
...
def build_session_final_statuses(
    ...
    for item in server_saved:
        slot = _session_asset_slot_for_paint_type(item.download_id.paint_type)
        if slot is not None:
            server_slots_by_key.setdefault(_session_status_key_for_download_id(session, item.download_id), set()).add(slot)
    for item in fallback_saved:
        slot = _session_asset_slot_for_paint_type(item.download_id.paint_type)
        if slot is not None:
            fallback_slots_by_key.setdefault(_session_status_key_for_download_id(session, item.download_id), set()).add(slot)
...
def _tp_showroom_link_for_scheme(cars: Iterable[dict], scheme_id: str) -> str:
    ...
    return f"https://www.tradingpaints.com/showroom/view/{wanted}"
        ...
        result.append(
            DownloadFile(
                download_id=DownloadId(
                    ...
                    is_numbered_car_spec=item.download_id.is_numbered_car_spec,
                ),
                url=item.url,
            )
        )
    ...
    *,
    ...
    try:
        ...
        if forced_retry_scheme_id:
            ...
            chosen_showroom_link = _tp_showroom_link_for_scheme(cars, chosen_scheme_id)
            reused_previous_scheme = False
            reused_recent_history = False
            ...
            chosen_showroom_link = _tp_showroom_link_from_car(chosen) or _tp_showroom_link_for_scheme(cars, chosen_scheme_id)
        ...
        if force_showroom_ui_activation:
            showroom_link = chosen_showroom_link or f"https://www.tradingpaints.com/showroom/view/{chosen_scheme_id}"
            write(
...
def process_session(
    ...
    def _emit_progress_rows() -> None:
        if progress_callback is None:
            return
        try:
            progress_callback(build_session_driver_rows(session, progress_statuses))
        except Exception:
    ...
    def _cancelled_result(
        ...
        server_saved_items = list(server_saved_items or [])
        fallback_saved_items = list(fallback_saved_items or [])
...
def start_session_cancel_monitor(
    *,
    expected_fingerprint: tuple[SessionId, tuple[tuple[int, str], ...]],
        ...
        try:
            ...
            while not stop_event.is_set() and not cancel_event.is_set():
                ...
                if poll_state in {SdkPollState.INACTIVE, SdkPollState.INVALID}:
                    ...
                    logging.info("Cancelling active paint pipeline because the iRacing session ended or became invalid.")
                    break
                if observed_session is not None and observed_session.fingerprint() != expected_fingerprint:
                    cancel_event.set()
                    logging.info("Cancelling active paint pipeline because the iRacing session changed while processing.")
                    break
                stop_event.wait(max(0.25, float(poll_seconds)))
        finally:
            if owned_reader:
...
class ConfigStore:
    ...
    def load(self) -> AppConfig:
        ...
        config.tp_detect_showroom_total_pages = bool(getattr(config, "tp_detect_showroom_total_pages", True))
    ...
    def save(self, config: AppConfig) -> None:
        ...
        safe.tp_detect_showroom_total_pages = bool(getattr(safe, "tp_detect_showroom_total_pages", True))
                ...
                if restored_saved:
                    ...
                    try:
                        processed_saved, monitor_snapshot, processed_rows = process_session(
                            ...
                            tp_detect_showroom_total_pages=bool(getattr(config, "tp_detect_showroom_total_pages", True)),
                            ...
                            progress_callback=lambda rows, _session=session, _existing=previous_rows, _replay=replay_mode_active: self._update_runtime_snapshot(
                                _session,
                                last_saved,
                                last_known_member_id,
                                session_rows=merge_session_driver_rows(_session, _existing, rows),
...
class DownloaderUI:
    def __init__(self, launched_from_autostart: bool = False) -> None:
        ...
        self.tp_detect_showroom_total_pages_var = tk.BooleanVar(value=bool(getattr(self.config, "tp_detect_showroom_total_pages", True)))
    ...
    def _build_ui(self) -> None:
        ...
        self.session_tree = ttk.Treeview(driver_list_frame, columns=("car", "suit", "helmet", "num", "ir", "lic", "sr", "name"), show="headings", height=18)
        ...
        ttk.Label(online_right, text="Built-in online behavior", font=("Segoe UI", 9, "bold"), foreground="#0b63b6").grid(row=0, column=0, sticky="w")
        ttk.Label(online_right, text="Nishizumi Paints now keeps the built-in fast online path enabled automatically. Legacy alternate speed settings are disabled behind the scenes, even if an older config still contains them.", justify="left", foreground="#176d2b", wraplength=430).grid(row=1, column=0, sticky="w", pady=(4, 0))
        ttk.Label(online_right, text="The app still keeps the normal backup and restore protection for the connected Trading Paints account while skipping the extra dashboard confirmation loop after each online showroom swap.", justify="left", foreground="#555555", wraplength=430).grid(row=2, column=0, sticky="w", pady=(8, 0))
        ttk.Checkbutton(online_right, text="Detect showroom total pages (slower)", variable=self.tp_detect_showroom_total_pages_var, command=self.on_setting_changed).grid(row=3, column=0, sticky="w", pady=(10, 0))
        ttk.Label(online_right, text="Optional. Detect the real total page count, then pick random pages from that real range.", justify="left", foreground="#555555", wraplength=430).grid(row=4, column=0, sticky="w", pady=(4, 0))
