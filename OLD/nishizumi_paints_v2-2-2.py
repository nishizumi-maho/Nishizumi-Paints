...
def show_launch_mode_dialog(saved_mode: str = "ask") -> tuple[str, bool] | None:
    ...
    ttk.Label(
        options,
        text="Uses less CPU/RAM and keeps running in the terminal window. Use --gui later if you want the full UI back.",
        foreground="#555555",
        justify="left",
        wraplength=430,
...
class DownloaderUI:
    ...
    def _build_ui(self) -> None:
        ...
        self.session_tree = ttk.Treeview(
            driver_list_frame,
            columns=("state", "num", "ir", "lic", "sr", "name"),
            show="headings",
            height=18,
        )
        ...
        self.session_tree.column("state", width=118, minwidth=108, anchor="center", stretch=False)
    ...
    def _refresh_session_tree(self) -> None:
        ...
        summary_bits = []
        for code in ("downloaded", "replay", "downloading", "fallback", "missing", "skipped"):
            if counts.get(code):
...
def main() -> int:
    args = parse_args()
    probe_lock = SingleInstanceLock(INSTANCE_MUTEX_NAME)
    if not probe_lock.acquire():
        SingleInstanceSignalServer.notify_existing_instance()
        return 0
    probe_lock.release()
    config_store = ConfigStore()
