...
def choose_and_sync_showroom_paint(
    *,
    ...
    try:
        ...
        if fast_mule_mode:
            write(f"Trading Paints showroom scheme {chosen_scheme_id} was requested for {raw_directory or matched_key}; MULE MODE is skipping the slow dashboard confirmation loop and polling the manifest directly.")
        elif fast_apply_mode:
            write(f"Trading Paints showroom scheme {chosen_scheme_id} was requested for {raw_directory or matched_key}; FAST MODE is skipping the per-fallback dashboard confirmation loop and polling the manifest directly.")
        else:
...
def choose_and_sync_showroom_driver_accessory(
    *,
    ...
    try:
        ...
        if fast_mule_mode:
            write(f"Trading Paints showroom {normalized_kind} scheme {chosen_scheme_id} was requested; MULE MODE is skipping the slow dashboard confirmation loop and polling the manifest directly.")
        elif fast_apply_mode:
            write(f"Trading Paints showroom {normalized_kind} scheme {chosen_scheme_id} was requested; FAST MODE is skipping the dashboard confirmation loop and polling the manifest directly.")
        else:
            confirm_deadline = time.monotonic() + 8.0
            chosen_scheme_confirmed = False
            while time.monotonic() < confirm_deadline:
    ...
    try:
        ...
        if effective_mule_mode:
            if touched_directory:
                logs.append(f"Trading Paints original scheme restore skipped for {display_directory} because Trading Paints MULE MODE is enabled for this profile.")
            return {
                'saved': fallback_saved,
                'logs': logs,
                'summary': summary,
    ...
    try:
        ...
        if effective_mule_mode:
            if touched_accessory:
                logs.append(f"Trading Paints original {normalized_kind} restore skipped because Trading Paints MULE MODE is enabled for this profile.")
            return {
                "saved": fallback_saved,
                "logs": logs,
                "summary": summary,
...
def apply_tp_showroom_fallbacks(
    ...
    effective_primary_fast_mode = bool(primary_fast_mode)
    effective_mule_mode = bool(mule_profile_mode)
    if effective_primary_fast_mode and effective_mule_mode:
        effective_primary_fast_mode = False
        logs.append("Both Trading Paints speed modes were enabled in config. Nishizumi Paints will keep MULE MODE and ignore FAST MODE for this online fallback run.")
    if effective_mule_mode:
        logs.append("Trading Paints MULE MODE is enabled for the online fallback: the app will skip original-scheme capture and final restore, reuse the showroom/browser context for the whole session, and the connected secondary account will keep the last loaded paint.")
    elif effective_primary_fast_mode:
        logs.append("Trading Paints FAST MODE is enabled for the online fallback: the app will keep the original-paint backup/restore flow, but it will skip the per-fallback dashboard verification loop after each random showroom swap and poll the manifest directly instead.")
...
def process_session(
    ...
    tp_primary_fast_mode: bool = False,
...
class ConfigStore:
    ...
    def load(self) -> AppConfig:
        ...
        config.tp_primary_fast_mode = bool(getattr(config, "tp_primary_fast_mode", False))
        config.tp_mule_profile_fast_mode = bool(getattr(config, "tp_mule_profile_fast_mode", False))
        if config.tp_primary_fast_mode and config.tp_mule_profile_fast_mode:
            config.tp_primary_fast_mode = False
    ...
    def save(self, config: AppConfig) -> None:
        ...
        safe.tp_primary_fast_mode = bool(getattr(safe, "tp_primary_fast_mode", False))
        safe.tp_mule_profile_fast_mode = bool(getattr(safe, "tp_mule_profile_fast_mode", False))
        if safe.tp_primary_fast_mode and safe.tp_mule_profile_fast_mode:
            safe.tp_primary_fast_mode = False
                ...
                if restored_saved:
                    ...
                    try:
                        processed_saved, monitor_snapshot, processed_rows = process_session(
                            ...
                            tp_primary_fast_mode=bool(getattr(config, "tp_primary_fast_mode", False)),
...
class DownloaderUI:
    def __init__(self, launched_from_autostart: bool = False) -> None:
        ...
        self.tp_primary_fast_mode_var = tk.BooleanVar(value=bool(getattr(self.config, "tp_primary_fast_mode", True)))
        self.tp_mule_fast_mode_var = tk.BooleanVar(value=bool(getattr(self.config, "tp_mule_profile_fast_mode", False)))
        if self.tp_primary_fast_mode_var.get() and self.tp_mule_fast_mode_var.get():
            self.tp_primary_fast_mode_var.set(False)
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Checkbutton(online_right, text="Use FAST MODE", variable=self.tp_primary_fast_mode_var, command=self.on_tp_primary_fast_mode_changed).grid(row=0, column=0, sticky="w")
        ttk.Label(online_right, text="Can be used with a primary account or a mule account. Nishizumi Paints saves the current paint state, restores the original paint at the end, and skips the slow dashboard verification loop after each fallback swap to gain speed.", justify="left", foreground="#176d2b", wraplength=430).grid(row=1, column=0, sticky="w", pady=(4, 0))
        ttk.Checkbutton(online_right, text="Use MULE MODE (Secondary account only!!!)", variable=self.tp_mule_fast_mode_var, command=self.on_tp_mule_fast_mode_changed).grid(row=2, column=0, sticky="w", pady=(10, 0))
        ttk.Label(online_right, text="This mode is only for a dedicated secondary mule account. Nishizumi Paints does not do the protection/restore checks here, so the mule account paint changes across sessions and is not restored, staying on the last loaded paint.", justify="left", foreground="#b00020", wraplength=430).grid(row=3, column=0, sticky="w", pady=(4, 0))
        ttk.Label(online_right, text="Do not enable both at the same time. Use FAST MODE or MULE MODE, not both together.", justify="left", foreground="#b26a00", wraplength=430).grid(row=4, column=0, sticky="w", pady=(8, 0))
        ttk.Checkbutton(online_right, text="Detect showroom total pages (slower)", variable=self.tp_detect_showroom_total_pages_var, command=self.on_setting_changed).grid(row=5, column=0, sticky="w", pady=(10, 0))
        ttk.Label(online_right, text="Optional. Detect the real total page count, then pick random pages from that real range.", justify="left", foreground="#555555", wraplength=430).grid(row=6, column=0, sticky="w", pady=(4, 0))
    ...
    def on_tp_primary_fast_mode_changed(self) -> None:
        if self.tp_primary_fast_mode_var.get() and self.tp_mule_fast_mode_var.get():
            ...
            try:
                self.messagebox.showwarning(APP_NAME, "Use only one Trading Paints speed mode at a time. Nishizumi Paints turned off MULE MODE automatically.", parent=self.root)
            except Exception:
                pass
        self.on_setting_changed()

    def on_tp_mule_fast_mode_changed(self) -> None:
        if self.tp_mule_fast_mode_var.get() and self.tp_primary_fast_mode_var.get():
            self.tp_primary_fast_mode_var.set(False)
            try:
                self.messagebox.showwarning(APP_NAME, "Use only one Trading Paints speed mode at a time. Nishizumi Paints turned off FAST MODE automatically.", parent=self.root)
            except Exception:
                pass
        self.on_setting_changed()
    ...
    def _read_ui_config(self) -> AppConfig:
        ...
        tp_primary_fast_mode = bool(self.tp_primary_fast_mode_var.get())
        tp_mule_fast_mode = bool(self.tp_mule_fast_mode_var.get())
        if tp_primary_fast_mode and tp_mule_fast_mode:
            tp_primary_fast_mode = False
        return AppConfig(
            ...
            tp_primary_fast_mode=tp_primary_fast_mode,
            tp_mule_profile_fast_mode=tp_mule_fast_mode,
    ...
    def on_setting_changed(self) -> None:
        ...
        if self._headless_controller_mode:
            ...
            self.service.update_config(self.config)
        if getattr(self.config, "tp_primary_fast_mode", False) and not getattr(self.config, "tp_mule_profile_fast_mode", False):
            logging.info("Trading Paints FAST MODE is enabled for the online fallback profile. It can be used on a primary or mule account while keeping original-paint backup/restore protection and skipping the per-fallback dashboard confirmation loop after each setScheme.")
        if getattr(self.config, "tp_mule_profile_fast_mode", False):
            logging.info("Trading Paints MULE MODE is enabled for the online fallback profile. The app will skip original-scheme capture and final restore, and the connected secondary account will keep the last loaded paint.")
    ...
    def _open_quick_start_wizard(self) -> None:
        ...
        self.ttk.Label(header, text="Recommended setup: normal window first, then Online fallback with a separate Trading Paints account and only one speed mode enabled at a time.", foreground="#176d2b").pack(anchor="w", pady=(4, 0))
        ...
        explain = (
            ...
            "Use FAST MODE if you want to keep backup and restore protection while skipping the slow dashboard verification loop. Use MULE MODE only on a dedicated secondary account if you want the fastest behavior and do not need restore protection."
        )
        ...
        self.ttk.Checkbutton(p4, text="Use FAST MODE", variable=self.tp_primary_fast_mode_var, command=self.on_tp_primary_fast_mode_changed).pack(anchor="w", pady=(12, 0))
        self.ttk.Label(p4, text="Can be used with a primary account or a mule account. It saves the current paint state, restores the original paint at the end, and skips the slow dashboard verification loop after each fallback swap.", wraplength=520, justify="left", foreground="#176d2b").pack(anchor="w", pady=(4, 0))
        self.ttk.Checkbutton(p4, text="Use MULE MODE (Secondary account only!!!)", variable=self.tp_mule_fast_mode_var, command=self.on_tp_mule_fast_mode_changed).pack(anchor="w", pady=(10, 0))
        self.ttk.Label(p4, text="Only for a dedicated secondary mule account. It does not do the protection/restore checks, so that account keeps the last loaded paint and is not restored at the end.", wraplength=520, justify="left", foreground="#b00020").pack(anchor="w", pady=(4, 0))
        self.ttk.Label(p4, text="Do not enable both at the same time. Use FAST MODE or MULE MODE, not both together.", wraplength=520, justify="left", foreground="#b26a00").pack(anchor="w", pady=(8, 0))
        ...
        self.ttk.Label(p5, text="Recommended final setup: Online fallback + separate Trading Paints account + choose FAST MODE or MULE MODE depending on whether you want restore protection.", wraplength=520, justify="left", foreground="#176d2b").pack(anchor="w", pady=(8, 0))
        pages.append(p5)
    ...
    def connect_tp_login(self, restart_on_success: bool = False, account_kind: str | None = None) -> None:
        ...
        if chosen_account_kind not in {"primary", "smurf"}:
            return
        if chosen_account_kind == "primary" and self.tp_mule_fast_mode_var.get():
            self.tp_mule_fast_mode_var.set(False)
            try:
                self.on_setting_changed()
            except Exception:
                logging.debug("Could not persist Trading Paints speed mode immediately after account choice", exc_info=True)
            self._append_log("Primary Trading Paints login selected while MULE MODE was enabled. Nishizumi Paints turned MULE MODE off automatically for protection.")
            try:
                self.messagebox.showwarning(
                    APP_NAME,
                    "You selected Primary account while MULE MODE was enabled. Nishizumi Paints turned MULE MODE off automatically for your protection.\n\nIf you really understand what will happen, you can enable MULE MODE again later. Otherwise, leave FAST MODE on so the app keeps backup and restore protection and you do not lose the original paint.",
                    parent=self.root,
                )
            except Exception:
                pass
        elif chosen_account_kind == "smurf" and self.tp_primary_fast_mode_var.get() and self.tp_mule_fast_mode_var.get():
            self.tp_primary_fast_mode_var.set(False)
            try:
                self.on_setting_changed()
            except Exception:
        ...
        self._append_log(f"Opening Trading Paints login in the persistent bundled/browser runtime for the selected {chosen_account_kind} account. Finish the login there, then click 'I logged in' in the app.")
