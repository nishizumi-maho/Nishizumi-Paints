...
class Session:
    ...
    def fingerprint(self) -> tuple[SessionId, tuple[tuple[int, str], ...]]:
        normalized_users = tuple(
            sorted((u.user_id, u.directory.lower()) for u in self.users)
        )
...
def _build_session_from_parts(weekend: object, driver_info: object) -> Session | None:
    ...
    if isinstance(drivers, list):
        for d in drivers:
            ...
            if user_id > 0 and isinstance(car_path, str) and car_path.strip():
                users.add(SessionUser(user_id=user_id, directory=car_path.strip()))
    ...
    return Session(session_id=SessionId(main_id, sub_id), users=users, local_user_id=local_user_id)
...
def save_path_for(download_id: DownloadId, paints_dir: Path) -> Path:
    ...
    if download_id.paint_type is PaintType.CAR:
        return paints_dir / download_id.directory / f"car_{uid}.tga"
    if download_id.paint_type is PaintType.CAR_DECAL:
        return paints_dir / download_id.directory / f"decal_{uid}.tga"
    if download_id.paint_type is PaintType.CAR_NUMBER:
        return paints_dir / download_id.directory / f"car_num_{uid}.tga"
    if download_id.paint_type is PaintType.CAR_SPEC:
        return paints_dir / download_id.directory / f"car_spec_{uid}.mip"
    if download_id.paint_type is PaintType.HELMET:
        return paints_dir / f"helmet_{uid}.tga"
    if download_id.paint_type is PaintType.SUIT:
        return paints_dir / f"suit_{uid}.tga"
...
def delete_saved(
    ...
    for f in files:
        if keep_user_id is not None and f.download_id.user_id == keep_user_id:
            kept.append(f)
        else:
            to_delete.append(f)
...
def _collect_wanted_downloads(
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=manifest_workers) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_user):
            ...
            completed += 1
            if not sync_my_livery_from_server and session.local_user_id is not None and user.user_id == session.local_user_id:
                logging.info("Manifest progress %s/%s • user %s skipped local driver sync", completed, total_users, user.user_id)
                continue
            try:
            ...
            user_wanted = [f for f in all_files if is_user_file(user, f)]
            logging.info(
                "Manifest progress %s/%s • user %s: %s matching files",
                completed,
                total_users,
                user.user_id,
                len(user_wanted),
            ...
            if not user_wanted and all_files:
                ...
                logging.debug(
                    "No matching car files for user %s (session dir='%s'). Manifest dirs=%s",
                    user.user_id,
                    user.directory,
                    manifest_dirs,
                )
            elif not all_files:
                logging.debug("Manifest returned no files for user %s", user.user_id)
...
class DownloaderService:
    ...
    def _cleanup_saved(
        ...
        keep_user_id = local_user_id if keep_my_livery_locally else None
        if not saved:
            return []
        logging.info(reason)
        kept = delete_saved(saved, keep_user_id=keep_user_id, paints_root=paints_root)
        if keep_user_id is not None and kept:
            logging.info("Kept %s local livery file(s) for your driver (user %s)", len(kept), keep_user_id)
        return kept
    ...
    def _run_loop(self) -> None:
        ...
        last_session_local_user_id: int | None = None
        ...
        try:
            while not self._stop_event.is_set():
                ...
                if self._clear_current_event.is_set():
                    ...
                    if last_saved:
                        last_saved = self._cleanup_saved(
                            ...
                            local_user_id=last_session_local_user_id,
                ...
                if poll_state in {SdkPollState.INACTIVE, SdkPollState.INVALID}:
                    ...
                    last_session_local_user_id = None
                    pending_reload = None
                ...
                last_session_local_user_id = session.local_user_id
...
class DownloaderUI:
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Separator(prefs, orient="horizontal").grid(row=3, column=0, columnspan=2, sticky="ew", pady=(8, 8))
        ttk.Label(prefs, text="Session", font=("Segoe UI", 9, "bold")).grid(row=4, column=0, columnspan=2, sticky="w")
        ttk.Checkbutton(prefs, text="Update my own paints", variable=self.sync_my_livery_var, command=self.on_setting_changed).grid(row=5, column=0, sticky="w")
        ttk.Checkbutton(prefs, text="Keep my livery locally", variable=self.keep_my_livery_var, command=self.on_setting_changed).grid(row=5, column=1, sticky="w", padx=(12, 0))
        ttk.Checkbutton(prefs, text="Delete downloaded livery", variable=self.cleanup_on_exit_var, command=self.on_setting_changed).grid(row=6, column=0, sticky="w")
        ttk.Checkbutton(prefs, text="Show activity", variable=self.show_activity_var, command=self.on_setting_changed).grid(row=6, column=1, sticky="w", padx=(12, 0))
        ttk.Checkbutton(prefs, text="Verbose logs", variable=self.verbose_var, command=self.on_setting_changed).grid(row=7, column=0, sticky="w", pady=(2, 0))
        ttk.Checkbutton(prefs, text="Show TP monitor", variable=self.show_tp_monitor_var, command=self.on_setting_changed).grid(row=7, column=1, sticky="w", padx=(12, 0), pady=(2, 0))
