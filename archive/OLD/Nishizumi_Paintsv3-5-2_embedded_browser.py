...
def default_iracing_documents_dir() -> Path:
    return Path.home() / "Documents" / "iRacing"
...
class DownloaderUI:
    ...
    def browse_iracing_documents_dir(self) -> None:
        initial_dir = resolve_iracing_documents_dir(self.iracing_documents_dir_var.get())
        try:
    ...
    def _open_quick_start_wizard(self) -> None:
        ...
        self.ttk.Checkbutton(p3, text="Use FAST MODE for primary account", variable=self.tp_primary_fast_mode_var, command=self.on_setting_changed).pack(anchor="w", pady=(10, 0))
        self.ttk.Label(p3, text="FAST MODE is separate from mule mode. It still restores the original paint state, but skips the slow dashboard confirmation loop after each fallback swap.", wraplength=520, justify="left", foreground="#176d2b").pack(anchor="w", pady=(4, 0))
