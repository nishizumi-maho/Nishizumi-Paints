...
def _apply_saved_file_statuses(
    ...
    for item in saved_items:
        key = session_target_key_from_download_id(item.download_id)
...
def build_session_stage_statuses(
    ...
    for item in wanted_items:
        ...
        if slot is None:
            continue
        wanted_slots_by_key.setdefault(session_target_key_from_download_id(item.download_id), set()).add(slot)
...
def build_session_final_statuses(
    ...
    for item in server_saved:
        slot = _session_asset_slot_for_paint_type(item.download_id.paint_type)
        if slot is not None:
            server_slots_by_key.setdefault(session_target_key_from_download_id(item.download_id), set()).add(slot)
    for item in fallback_saved:
        slot = _session_asset_slot_for_paint_type(item.download_id.paint_type)
        if slot is not None:
            fallback_slots_by_key.setdefault(session_target_key_from_download_id(item.download_id), set()).add(slot)
...
def apply_local_tp_random_fallbacks(
    ...
    for user in sorted(session.users, key=lambda u: (u.is_ai, u.directory.lower(), u.user_id, u.team_id or 0)):
        ...
        need_helmet = bool(random_helmets) and helmet_enabled_for_user and target_key not in existing_helmet_targets
        need_suit = bool(random_suits) and suit_enabled_for_user and target_key not in existing_suit_targets
        ...
        if applied_items:
            ...
            for item in applied_items:
                ...
                if item.download_id.paint_type is PaintType.CAR:
                    ...
                    existing_helmet_targets.add(key)
                elif item.download_id.paint_type is PaintType.SUIT:
                    existing_suit_targets.add(key)
        else:
...
def choose_and_sync_showroom_driver_accessory(
    *,
    ...
    try:
        ...
        if fast_mule_mode:
            write(f"Trading Paints showroom {normalized_kind} scheme {chosen_scheme_id} was requested; TP mule fast mode is skipping the slow dashboard confirmation loop and polling the manifest directly.")
        elif fast_apply_mode:
    ...
    try:
        ...
        if effective_mule_mode:
            if touched_accessory:
                logs.append(f"Trading Paints original {normalized_kind} restore skipped because TP mule fast mode is enabled for this profile.")
            return {
                "saved": fallback_saved,
                "logs": logs,
                "summary": summary,
...
def apply_tp_showroom_fallbacks(
    ...
    for candidate in sorted(session.users, key=lambda u: (u.is_ai, u.directory.lower(), u.user_id, u.team_id or 0)):
        ...
        if bool(random_helmets) and helmet_enabled_for_candidate and target_key not in existing_helmet_targets:
            helmet_users.append(candidate)
        if bool(random_suits) and suit_enabled_for_candidate and target_key not in existing_suit_targets:
...
def process_session(
    ...
    def _on_saved_items(_downloaded: DownloadedFile, saved_items: list[SavedFile]) -> None:
        _apply_saved_file_statuses(progress_statuses, saved_items, "downloaded")
        _emit_progress_rows()
    ...
    def _on_fallback_saved_items(saved_items: list[SavedFile], status_code: str) -> None:
        _apply_saved_file_statuses(progress_statuses, saved_items, status_code)
        _emit_progress_rows()
        if reload_debouncer is not None:
            reload_debouncer.request_saved_items(saved_items)
...
class DownloaderUI:
    def __init__(self, launched_from_autostart: bool = False) -> None:
        ...
        self.tp_primary_fast_mode_var = tk.BooleanVar(value=bool(getattr(self.config, "tp_primary_fast_mode", False)))
    ...
    def _build_ui(self) -> None:
        ...
        self.session_tree = ttk.Treeview(driver_list_frame, columns=("state", "car", "suit", "helmet", "num", "ir", "lic", "sr", "name"), show="headings", height=18)
        ...
        self.session_tree.column("state", width=150, minwidth=138, anchor="center", stretch=False)
        self.session_tree.column("car", width=128, minwidth=118, anchor="center", stretch=False)
        self.session_tree.column("suit", width=128, minwidth=118, anchor="center", stretch=False)
        self.session_tree.column("helmet", width=128, minwidth=118, anchor="center", stretch=False)
        ...
        self.session_tree.column("name", width=280, minwidth=220, anchor="w", stretch=False)
        ...
        ttk.Label(prefs, text="iRacing Documents folder").grid(row=4, column=0, sticky="w", pady=(8, 0))
        iracing_docs_row = ttk.Frame(prefs)
        iracing_docs_row.grid(row=5, column=0, columnspan=2, sticky="ew", pady=(4, 0))
        ...
        ttk.Label(prefs, text="Folder that contains paint, replay, and airosters. Use the button to change it. When you confirm a new location, the app restarts to apply the change.", foreground="#666666", wraplength=520, justify="left").grid(row=6, column=0, columnspan=2, sticky="w", pady=(4, 0))
        ttk.Separator(prefs, orient="horizontal").grid(row=7, column=0, columnspan=2, sticky="ew", pady=(8, 8))
        ttk.Label(prefs, text="Session", font=("Segoe UI", 9, "bold"), foreground="#0b63b6").grid(row=8, column=0, columnspan=2, sticky="w")
        ttk.Checkbutton(prefs, text="Update my own paints", variable=self.sync_my_livery_var, command=self.on_setting_changed).grid(row=9, column=0, sticky="w")
        ttk.Checkbutton(prefs, text="Keep my livery locally", variable=self.keep_my_livery_var, command=self.on_setting_changed).grid(row=9, column=1, sticky="w", padx=(12, 0))
        ttk.Checkbutton(prefs, text="Delete live session paints", variable=self.cleanup_on_exit_var, command=self.on_setting_changed).grid(row=10, column=0, sticky="w")
        ...
        experimental_top = ttk.LabelFrame(experimental_tab, text="Experimental online fallback options", padding=10)
        experimental_top.grid(row=0, column=0, sticky="ew")
        ttk.Label(
            experimental_top,
            text=(
                "These options are optional and more advanced. FAST MODE is meant for primary-account online fallback: "
                "it still captures your original local paint state and restores it at the end, but it skips the slow dashboard confirmation loop after each random showroom swap."
            ),
            foreground="#444444",
            justify="left",
            wraplength=920,
        ...
        ttk.Checkbutton(
            experimental_top,
            text="FAST MODE (primary account only)",
            variable=self.tp_primary_fast_mode_var,
            command=self.on_setting_changed,
        ).pack(anchor="w", pady=(12, 0))
        ttk.Label(
            experimental_top,
            text="Not the same as Mule Mode. FAST MODE keeps backup/restore protection for the connected primary account and only skips the per-fallback dashboard verification step.",
            foreground="#176d2b",
            justify="left",
            wraplength=920,
    ...
    def on_setting_changed(self) -> None:
        ...
        if getattr(self.config, "tp_primary_fast_mode", False) and not getattr(self.config, "tp_mule_profile_fast_mode", False):
            logging.info("Trading Paints FAST MODE is enabled for the online fallback profile. The app will keep original-paint backup/restore protection, but it will skip the per-fallback dashboard confirmation loop after each setScheme.")
    ...
    def _open_quick_start_wizard(self) -> None:
        ...
        self.ttk.Checkbutton(p4, text="Use TP mule fast mode", variable=self.tp_mule_fast_mode_var, command=self.on_setting_changed).pack(anchor="w", pady=(12, 0))
        self.ttk.Label(p4, text="Use mule fast mode only for a secondary mule account. It skips restore steps to be faster.", wraplength=520, justify="left", foreground="#b00020").pack(anchor="w", pady=(4, 0))
        self.ttk.Checkbutton(p4, text="Use FAST MODE for primary account", variable=self.tp_primary_fast_mode_var, command=self.on_setting_changed).pack(anchor="w", pady=(10, 0))
        self.ttk.Label(p4, text="FAST MODE is separate from mule mode. It still restores the original paint state, but skips the slow dashboard confirmation loop after each fallback swap.", wraplength=520, justify="left", foreground="#176d2b").pack(anchor="w", pady=(4, 0))
        ...
        self.ttk.Label(p5, text="Recommended final setup: Online fallback + secondary mule Trading Paints account + mule fast mode enabled.", wraplength=520, justify="left", foreground="#176d2b").pack(anchor="w", pady=(8, 0))
        pages.append(p5)
    ...
    def connect_tp_login(self, restart_on_success: bool = False, account_kind: str | None = None) -> None:
        ...
        elif chosen_account_kind == "primary":
            self.tp_mule_fast_mode_var.set(False)
            try:
                self.on_setting_changed()
            except Exception:
                logging.debug("Could not persist primary-account TP mode immediately after account choice", exc_info=True)
