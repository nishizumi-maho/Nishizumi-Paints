...
class DownloaderUI:
    ...
    def _open_quick_start_wizard(self) -> None:
        ...
        self.ttk.Label(p3, text="Choose who should receive a backup paint when Trading Paints has no paint for them.", wraplength=520, justify="left", foreground="#444444").pack(anchor="w", pady=(8, 0))
        self.ttk.Checkbutton(p3, text="Fallback for real drivers without TP paints", variable=self.random_real_var, command=self.on_random_fallback_targets_changed).pack(anchor="w", pady=(10, 0))
        self.ttk.Checkbutton(p3, text="Fallback for AI drivers without TP paints", variable=self.random_ai_var, command=self.on_random_fallback_targets_changed).pack(anchor="w", pady=(6, 0))
