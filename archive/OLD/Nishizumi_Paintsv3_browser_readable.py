...
Open-source oriented single-file build of the Nishizumi Paints desktop app.

This script intentionally keeps the full application in one file so it can be
run directly and packaged easily, but the code is organized in large sections
that map to the app's main responsibilities:

1. App metadata, constants, and path helpers
2. Browser / Trading Paints login helpers
3. Launch-mode, single-instance, and headless-control helpers
4. Domain models used by the downloader and UI
5. Logging, filesystem, HTTP, and SDK utilities
6. Random-pool, replay-pack, and AI-roster workflows
7. Trading Paints showroom / online fallback workflow
8. Core download, save, and session-processing pipeline
9. Configuration storage, background service, tray integration, and Tk UI
10. Application entry points for GUI and headless modes

The goal of this version is readability for open-source readers:
- clearer section boundaries
- more descriptive docstrings on major types and entry points
- comments around the non-obvious workflows
- minimal behavior changes compared with the original script
"""
...
# ============================================================================
# APP METADATA AND GLOBAL CONSTANTS
# ============================================================================
#
# Open-source navigation tip:
# - Search for "APPLICATION ENTRY POINTS" to find startup flow.
# - Search for "DownloaderService" to find the background runtime.
# - Search for "DownloaderUI" to find the Tk interface.
# - Search for "process_session" to find the main download pipeline.
# - Search for "TRADING PAINTS SHOWROOM / ONLINE FALLBACK WORKFLOW" to find
#   the browser-driven online fallback logic.
...
# ============================================================================
# PATH HELPERS, BROWSER DISCOVERY, AND AUTH STATE PERSISTENCE
# ============================================================================
def resource_path(name: str) -> Path:
    """Return a file path that works both from source and frozen builds."""
    base = Path(getattr(sys, "_MEIPASS", Path(__file__).resolve().parent))
    return base / name

def app_runtime_dir() -> Path:
    """Return the directory that should be treated as the app runtime root."""
...
def _embedded_tp_browser_paths() -> list[Path]:
    """Search for a bundled Chromium/Chrome runtime shipped with the app.

    The packaged EXE can carry a portable browser next to the executable.
    Keeping this logic in one helper makes the browser fallback order easier to
    understand and easier to tweak later.
    """
...
def resolve_tp_browser_launch_target(browser_channel: str | None = "chrome") -> dict[str, object]:
    """Resolve which browser runtime Playwright should use for TP auth."""
...
def launch_tp_persistent_context(
    ...
    **kwargs,
    ...
    """Launch a persistent Playwright browser context using the resolved target."""
...
def write_tp_login_status(profile_dir: Path | None = None, ok: bool = True, message: str = "") -> None:
    """Persist the last known Trading Paints login state for the browser profile."""
    try:
...
def _tp_context_looks_authenticated(context, open_pages=None) -> tuple[bool, str]:
    """Best-effort check used after login to confirm a TP browser profile is usable.

    The Trading Paints workflow is browser-driven, so we inspect page content and
    also issue authenticated requests from the Playwright context when needed.
    """
...
def show_launch_mode_dialog(saved_mode: str = "ask") -> tuple[str, bool] | None:
    """Display the startup dialog that lets the user choose GUI/headless mode."""
...
# ============================================================================
# VERSIONING, UPDATE CHECKS, AND PROCESS COORDINATION
# ============================================================================

@dataclass(frozen=True)
class GitHubReleaseInfo:
    """Minimal normalized representation of the newest GitHub release."""
    tag_name: str
    html_url: str
...
def fetch_latest_github_release() -> GitHubReleaseInfo:
    """Fetch and normalize the newest release published on GitHub."""
...
class SingleInstanceLock:
    """Windows mutex wrapper used to prevent multiple app instances."""
...
class SingleInstanceSignalServer:
    """Small localhost server used to wake/focus an already-running instance."""
...
class HeadlessControlServer:
    """Local control socket that lets the GUI talk to a headless service instance."""
    def __init__(
        self,
...
# ============================================================================
# DOMAIN MODELS AND SESSION SNAPSHOTS
# ============================================================================

class PaintType(str, Enum):
    """Known Trading Paints / iRacing asset categories handled by the app."""
    CAR = "car"
    CAR_DECAL = "car_decal"
    CAR_NUMBER = "car_num"
    CAR_SPEC = "car_spec"
...
@dataclass(frozen=True)
class SessionId:
    """Compact identifier for the active iRacing session/subsession pair."""
    main_session_id: int
...
@dataclass(frozen=True)
class SessionUser:
    """Normalized view of one driver/team entry present in the session."""
    user_id: int
    directory: str
...
@dataclass(frozen=True)
class Session:
    """Normalized session snapshot used by the downloader pipeline."""
...
class TransferBatchMonitor:
    """Collect timing and throughput stats for one manifest/download/save batch."""
...
# ============================================================================
# CLI ARGUMENTS, LOGGING, AND GENERIC RUNTIME HELPERS
# ============================================================================
def parse_args() -> argparse.Namespace:
    """Parse CLI options for both source execution and packaged EXE usage."""
...
def setup_logging(verbose: bool) -> None:
    """Configure the process-wide logger used by both GUI and headless modes."""
...
# ============================================================================
# SESSION PARSING, HTTP FETCHING, AND RANDOM-POOL UTILITIES
# ============================================================================
...
@dataclass
# ============================================================================
# TRADING PAINTS SHOWROOM / ONLINE FALLBACK WORKFLOW
# ============================================================================

class TPShowroomSyncResult:
    """Outcome of one online-showroom fallback sync attempt."""
    ok: bool
...
def choose_and_sync_showroom_paint(
    # Core of the browser-driven online fallback flow.
    *,
    directory: str,
    mapping_path: Path | None,
    profile_dir: Path,
...
def apply_tp_showroom_fallbacks(
    # Applies online fallback only to users still missing usable paints after
    # the normal manifest/download path finishes.
    session: Session,
    saved: list[SavedFile],
    paints_dir: Path,
    enable_real_drivers: bool,
    ...
    if sync_playwright is not None:
        try:
            ...
            shared_tp_context, _launch_target = launch_tp_persistent_context(
                shared_tp_browser.chromium,
                user_data_dir=str(profile_dir),
...
def _unique_local_ai_roster_dir(ai_rosters_dir: Path, preferred_name: str) -> Path:
    ...
    while True:
        ...
        suffix += 1
# ============================================================================
# AI ROSTER WORKFLOW AND CONTEXTUAL MANIFEST RESOLUTION
# ============================================================================
...
def sync_ai_rosters(member_id: int, ai_rosters_dir: Path, retries: int, retry_backoff_seconds: float, do_cleanup: bool = True) -> AIRosterSyncResult:
    """Mirror AI rosters from Trading Paints into the local cache directory."""
...
def fetch_context_files(
    ...
    """Fetch context-aware TP manifest files when the session provides extra hints."""
...
# ============================================================================
# FILE NAMING, DOWNLOAD/SAVE PIPELINE, AND REPLAY PACK MANAGEMENT
# ============================================================================
def normalize_directory(value: str) -> str:
    """Normalize car/track directory strings so matching stays predictable."""
...
def download_file(
    ...
    """Download one TP asset into the temp staging area."""
...
def move_or_extract(
    ...
    """Install a staged file into its final location, unpacking .bz2 when needed."""
...
def process_session(
    ...
    """Run the full manifest -> download -> save -> fallback pipeline for one session."""
...
# ============================================================================
# SDK ACCESS, TEXTURE RELOADS, AND CONFIG STORAGE
# ============================================================================
...
@dataclass
class AppConfig:
    """Persistent user configuration shared by GUI and headless modes."""
...
class ConfigStore:
    """Read/write wrapper for the JSON settings file."""
...
class QueueLogHandler(logging.Handler):
    """Logging handler that forwards formatted log lines into the Tk UI queue."""
...
# ============================================================================
# BACKGROUND SERVICE, TRAY INTEGRATION, AND TKINTER UI
# ============================================================================

class DownloaderService:
    """Background coordinator that watches sessions and runs the downloader pipeline.

    Keep this class mostly free of Tk code: it is the runtime engine used by both
    the full GUI and the headless service mode.
    """
    def __init__(self, config: AppConfig, log_queue: "queue.Queue[str] | None" = None) -> None:
        ...
        # RuntimeSnapshot is the main service -> UI handoff object. The UI polls
        # it instead of reaching into lots of service internals directly.
...
class WindowsTrayIcon:
    """Thin Windows tray-icon wrapper used by the Tk desktop app."""
...
class DownloaderUI:
    """Tkinter front-end for configuring the service and watching live status.

    This class owns widget creation, UI state variables, and user-facing actions.
    The background work remains in DownloaderService so the logic can still be
    reused from headless mode.
    """
    def __init__(self, launched_from_autostart: bool = False) -> None:
        ...
        # The service owns the runtime logic. The UI only configures it and
        # displays state/log snapshots. Keeping that boundary clear makes the
        # script much easier to split into modules later.
...
def run_headless(args: argparse.Namespace | None = None) -> int:
    """Start the service without the Tk window and keep it attached to the console."""
    if args is None:
        args = parse_args()
...
def resolve_requested_launch_mode(args: argparse.Namespace, config_store: ConfigStore) -> str | None:
    """Decide whether this process should start in GUI or headless mode."""
...
def main() -> int:
    """Main process entry point. Dispatches to GUI or headless mode."""
    args = parse_args()
    config_store = ConfigStore()
