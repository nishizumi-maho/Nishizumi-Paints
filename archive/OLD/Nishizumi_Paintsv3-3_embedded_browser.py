...
def _copy_source_file_to_saved_targets(source_path: Path | None, session_id: SessionId, download_id: DownloadId, paints_dir: Path) -> list[SavedFile]:
    ...
    return saved
def _apply_local_tp_random_fallback_for_user(
    *,
    session: Session,
    user: User,
    paints_dir: Path,
...
) -> tuple[list[SavedFile], str | None, bool]:
    ...
    if entry is None:
        ...
        return [], f"TP local random fallback had no usable source for {'AI' if user.is_ai else 'driver'} {label} on {user.directory}.", False
    label = user.display_name or (f"AI {target_id}" if user.is_ai else f"user {target_id}")
    base_kwargs = {
        "user_id": target_id,
        "directory": user.directory,
    ...
    if not saved_items:
        return [], f"TP local random fallback picked a source for {'AI' if user.is_ai else 'driver'} {label} on {user.directory}, but nothing could be saved into the target paint slots.", False
    ...
    log_line = (
        f"TP local random fallback applied to {'AI' if user.is_ai else 'driver'} {label} using {entry.source_kind} source {entry.source_id} "
        f"for {user.directory}; source_files=({' | '.join(source_parts)}); saved_targets=[{save_targets}]"
    )
    return saved_items, log_line, reused_existing_source
    ...
    for user in sorted(session.users, key=lambda u: (u.is_ai, u.directory.lower(), u.user_id, u.team_id or 0)):
        ...
        for paint_type, source_path in (
            ...
            if copied:
                ...
                applied_items.extend(copied)
                added_any = True
        if added_any:
            if saved_callback is not None and applied_items:
                try:
            ...
            if not result.ok:
                if getattr(result, 'force_local_fallback', False):
                    ...
                    with local_fallback_lock:
                        local_saved_items, local_log, local_reused_source = _apply_local_tp_random_fallback_for_user(
                            session=session,
                            user=user,
                            paints_dir=paints_dir,
...
def apply_tp_showroom_fallbacks(
    ...
    if effective_mule_mode:
        ...
        logs.append('Trading Paints FAST MODE is enabled for the online fallback: the app will keep the original-paint backup/restore flow, but it will skip the per-fallback dashboard confirmation loop after each setScheme and poll the manifest directly instead.')
    ...
    if not lane_users_by_directory:
        return fallback_saved, logs, summary
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_lanes, thread_name_prefix='TPFallbackLane') as executor:
        future_map = {}
        for lane_index, (directory_key, users) in enumerate(ordered_lanes):
            initial_cache_entry = original_scheme_cache.get(directory_key)
            if not isinstance(initial_cache_entry, dict):
                initial_cache_entry = None
            lane_kwargs = dict(
                ...
                retry_backoff_seconds=retry_backoff_seconds,
                saved_callback=saved_callback,
                initial_original_cache_entry=initial_cache_entry,
            ...
            if sync_playwright is not None:
                ...
                future = executor.submit(
                    _process_tp_showroom_fallback_lane,
                    **lane_kwargs,
                )
            future_map[future] = directory_key

        for future in concurrent.futures.as_completed(future_map):
            directory_key = future_map[future]
            try:
                lane_result = future.result()
            except Exception as exc:  # noqa: BLE001
                logging.debug("Trading Paints fallback lane crashed for %s", directory_key, exc_info=True)
                summary.missing_source_targets += len(lane_users_by_directory.get(directory_key, []))
                logs.append(f"Trading Paints fallback lane crashed for {directory_key}: {exc}")
                continue
            ...
            if updated_recent:
                recent_schemes_by_directory[directory_key] = updated_recent
            updated_cache_entry = lane_result.get('original_cache_entry')
            if isinstance(updated_cache_entry, dict) and str(updated_cache_entry.get('scheme_id') or '').strip():
                original_scheme_cache[directory_key] = dict(updated_cache_entry)

    try:
        save_tp_recent_schemes(recent_schemes_by_directory)
    except Exception:
        logging.debug('Could not save Trading Paints recent schemes cache', exc_info=True)
    try:
        save_tp_original_scheme_cache(original_scheme_cache, profile_dir)
    except Exception:
        logging.debug('Could not save Trading Paints original scheme cache', exc_info=True)
    return fallback_saved, logs, summary
