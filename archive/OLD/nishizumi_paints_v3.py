...
class DownloaderUI:
    ...
    def _finish_update_check(self, release: GitHubReleaseInfo | None, error: str | None, manual: bool) -> None:
        ...
        if comparison > 0:
            ...
            if manual:
                self.self.messagebox.showinfo(APP_NAME, f"You are already on the latest version.\n\nCurrent version: {APP_VERSION}")
        else:
            ...
            if manual:
                self.self.messagebox.showinfo(APP_NAME, f"Your current build ({APP_VERSION}) is newer than the latest public GitHub release ({release.tag_name}).")
        if self.check_updates_var.get() and not self._exiting:
            self._schedule_update_check(initial=False)
