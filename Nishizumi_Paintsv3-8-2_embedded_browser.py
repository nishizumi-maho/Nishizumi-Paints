...
def _find_session_user_for_download_id(session: Session | None, download_id: DownloadId) -> SessionUser | None:
    ...
    for user in session.users:
        target_kind = "team" if user.team_id is not None and user.team_id > 0 else "user"
        target_id = int(user.team_id if target_kind == "team" else user.user_id)
...
def normalize_tp_fallback_lanes_mode(value: str) -> str:
    mode = str(value or "session_total").strip().lower().replace("-", "_").replace(" ", "_")
    return mode if mode in {"session_total", "manual"} else "session_total"
...
def apply_local_tp_random_fallbacks(
    ...
    for user in sorted(session.users, key=lambda u: (u.is_ai, u.directory.lower(), u.user_id, u.team_id or 0)):
        ...
        helmet_enabled_for_user = bool(random_helmets_ai) if user.is_ai else bool(random_helmets_real_drivers)
        suit_enabled_for_user = bool(random_suits_ai) if user.is_ai else bool(random_suits_real_drivers)
    ...
    *,
    ...
    while time.monotonic() < deadline:
        ...
        try:
            ...
            time.sleep(0.9)
            continue
        filtered = _tp_manifest_items_for_accessory(manifest, paint_type)
        if filtered:
    ...
    *,
    ...
    try:
        ...
        if (not fast_mule_mode) and not captured_original_scheme_id:
            ...
            if captured_original_scheme_id:
                write(f"Captured original Trading Paints {normalized_kind} scheme: {captured_original_scheme_id} ({capture_source})")
            elif baseline_url_map:
                protection_message = (
    ...
    *,
    ...
    try:
        for index, user in enumerate(users):
            ...
            target_id = int(user.team_id) if target_is_team else int(user.user_id)
            label = user.display_name or (f"AI {target_id}" if user.is_ai else f"user {target_id}")
            remaining_targets = max(1, len(users) - index)
            result = None
            attempt_excluded_scheme_ids = set(used_scheme_ids)
            max_online_apply_attempts = 3
            for online_apply_attempt in range(1, max_online_apply_attempts + 1):
                try:
                ...
                if result.chosen_scheme_id:
                    attempt_excluded_scheme_ids.add(str(result.chosen_scheme_id).strip())
                should_retry_online_apply = bool((not result.ok) and result.set_scheme_applied and online_apply_attempt < max_online_apply_attempts)
            ...
            if not result.ok:
                if getattr(result, 'force_local_fallback', False) or bool(result.set_scheme_applied):
            ...
            if result.direct_items:
                current_manifest_urls = _tp_manifest_url_map(result.direct_items)
    ...
    *,
    ...
    try:
        ...
        for user in users:
            ...
            target_id = int(user.team_id) if target_is_team else int(user.user_id)
            label = user.display_name or (f"AI {target_id}" if user.is_ai else f"user {target_id}")
            remaining_targets = max(1, len(users) - len(used_scheme_ids))
            result = None
            attempt_excluded_scheme_ids = set(used_scheme_ids)
            max_online_apply_attempts = 3
            for online_apply_attempt in range(1, max_online_apply_attempts + 1):
            ...
            if not result.ok:
                if bool(result.set_scheme_applied):
                    logs.append(
...
def apply_tp_showroom_fallbacks(
    ...
    if effective_primary_fast_mode and effective_mule_mode:
        ...
        logs.append("Both Trading Paints speed modes were enabled in config. Nishizumi Paints will keep FAST MODE and ignore MULE MODE for safety.")
    if effective_mule_mode:
    ...
    for candidate in sorted(session.users, key=lambda u: (u.is_ai, u.directory.lower(), u.user_id, u.team_id or 0)):
        ...
        accessory_target_key = (target_is_team, target_id)
        if target_key not in existing_car_targets:
        ...
        helmet_enabled_for_candidate = bool(random_helmets_ai) if candidate.is_ai else bool(random_helmets_real_drivers)
        suit_enabled_for_candidate = bool(random_suits_ai) if candidate.is_ai else bool(random_suits_real_drivers)
    ...
    manual_lane_cap = max(1, min(100, positive_int(tp_manual_max_lanes, 3)))
    if lane_group_count > 0:
        ...
        max_lanes = 0
    if helmet_users:
        logs.append(f"Trading Paints online accessory lane active: helmet targets={len(helmet_users)}.")
    if suit_users:
        logs.append(f"Trading Paints online accessory lane active: suit targets={len(suit_users)}.")
        ...
        for future in concurrent.futures.as_completed(future_map):
            ...
            try:
                ...
                if task_kind == "car":
                    ...
                    summary.missing_source_targets += len(helmet_users)
                elif task_kind == "suit":
                    summary.missing_source_targets += len(suit_users)
...
def _save_downloads_parallel(
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=save_workers) as executor:
        ...
        for index, downloaded in enumerate(downloads, start=1):
            future = executor.submit(move_or_extract, downloaded, paints_dir, batch_monitor, retries, retry_backoff_seconds)
...
class ConfigStore:
    ...
    def load(self) -> AppConfig:
        ...
        config.tp_fallback_lanes_mode = normalize_tp_fallback_lanes_mode(getattr(config, "tp_fallback_lanes_mode", "session_total"))
        config.tp_manual_max_lanes = max(1, min(100, positive_int(getattr(config, "tp_manual_max_lanes", 3), 3)))
    ...
    def save(self, config: AppConfig) -> None:
        ...
        safe.tp_manual_max_lanes = max(1, min(100, positive_int(getattr(safe, "tp_manual_max_lanes", 3), 3)))
            ...
            while not self._stop_event.is_set():
                ...
                if restored_saved:
                    ...
                    processed_saved, monitor_snapshot, processed_rows = process_session(
                        ...
                        tp_fallback_lanes_mode=getattr(config, "tp_fallback_lanes_mode", "session_total"),
                        tp_manual_max_lanes=int(getattr(config, "tp_manual_max_lanes", 3) or 3),
...
class DownloaderUI:
    def __init__(self, launched_from_autostart: bool = False) -> None:
        ...
        self.tp_fallback_lanes_mode_var = tk.StringVar(value=tp_fallback_lanes_mode_label(getattr(self.config, "tp_fallback_lanes_mode", "session_total")))
        self.tp_manual_max_lanes_var = tk.IntVar(value=max(1, min(100, int(getattr(self.config, "tp_manual_max_lanes", 3) or 3))))
    ...
    def _build_ui(self) -> None:
        ...
        self.tp_fallback_lanes_mode_combo = ttk.Combobox(perf, state="readonly", values=["Session Total", "Manual"], width=14, textvariable=self.tp_fallback_lanes_mode_var)
        ...
        ttk.Label(perf, text="Session Total = one lane for every distinct car model still missing a paint in the current session. Manual = cap the maximum lane count yourself.", foreground="#666666", wraplength=320, justify="left").grid(row=9, column=0, columnspan=2, sticky="w", pady=(8, 0))
    ...
    def _refresh_tp_manifest_member_id_summary(self) -> None:
        ...
        if override_id > 0:
            self.tp_manifest_member_id_summary_var.set(
                f"Override active: {override_id}. The app will treat this as the iRacing member ID of the other account that is downloading paints, and online fallback will poll fetch_user for that account instead of your current racing account."
            )
        else:
            self.tp_manifest_member_id_summary_var.set(
                "No override active. The app is using the iRacing member ID of the account currently racing in the session."
            )
    ...
    def on_tp_manual_max_lanes_changed(self) -> None:
        try:
            ...
            value = max(1, min(100, int(getattr(self.config, "tp_manual_max_lanes", 3) or 3)))
    ...
    def _read_ui_config(self) -> AppConfig:
        ...
        try:
            ...
            tp_manual_max_lanes = max(1, min(100, int(getattr(self.config, "tp_manual_max_lanes", 3) or 3)))
